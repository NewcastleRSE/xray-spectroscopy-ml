...
from xanesnet.utils import load_descriptors, load_model_list
...
def init_scheme(model_dir, mode):
    # Load metadata
...
def init_model_list(model_dir):
    return load_model_list(Path(model_dir))
...
class TestAEPredict:
    ...
    def test_std_xanes(self):
        model_dir = "models/model_ae_xanes"
        scheme = init_scheme(model_dir, "predict_xyz")
        model = init_model(model_dir)
        # Check prediction result
        result = scheme.predict_std(model)
