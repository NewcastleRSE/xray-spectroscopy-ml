...
# target std, prediction std, best-percent prediction mean, worst-percent
# prediction mean.
_MethodMeans = tuple[list[str], str, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]
...
class MeanSpectrumPlotter(Plotter):
    ...
    the mean prediction of the ``percent`` best and worst samples ranked by
    error. ``spread.pdf`` compares the per-channel standard deviation of the
    ...
    Args:
        ...
        percent: Percentage of samples in the best and worst prediction groups.
    ...
    def __init__(self, plotter_type: str, percent: float = 20.0, sort_key: str | None = None) -> None:
    ...
    def plot(self, results: AnalysisResults, output_dir: Path) -> None:
        ...
        for reader_idx, reader_selectors in enumerate(results.selectors):
            ...
            for sel_idx, selector in enumerate(reader_selectors):
                ...
                means = self._collect_means(selector, stream)
                if means is None:
                    continue

                colour = method_colour(len(methods))
                methods.append((label_lines, colour, *means))
                ...
                target_mean, pred_mean, target_std, pred_std, best_mean, worst_mean = means
        ...
        self._combined_tail_overlay(methods, combined_dir / "tail_best_overlay.pdf", "best")
        self._combined_tail_overlay(methods, combined_dir / "tail_worst_overlay.pdf", "worst")
    ...
    def _collect_means(
        ...
        """Compute per-channel mean statistics for one method.
        ...
        Returns:
            ``(target_mean, pred_mean, target_std, pred_std, best_mean, worst_mean)``
            curves, or ``None`` when no samples are selected.
        """
        entries: list[_Entry] = []
        ...
        preds = np.stack([np.asarray(sample["prediction"]).ravel() for sample, _, _ in entries])
        targets = np.stack([np.asarray(sample["target"]).ravel() for sample, _, _ in entries])

        n_tail = max(1, int(np.ceil(len(entries) * self.percent / 100.0)))
        order = np.argsort([entry[2] for entry in entries])
        best_preds = preds[order[:n_tail]]
        worst_preds = preds[order[-n_tail:]]
        ...
        return (
            ...
            targets.std(axis=0),
            preds.std(axis=0),
            best_preds.mean(axis=0),
            worst_preds.mean(axis=0),
        )
    ...
    def _mean_figure(
        ...
        ax.plot(x, target_mean, color="black", linewidth=1.6, label="Target mean")
        ax.fill_between(
            x,
            pred_mean - pred_std,
            pred_mean + pred_std,
    ...
    def _tail_figure(
        ...
        fig, ax = plt.subplots(figsize=(6.5, 3.6))
        x = np.arange(len(pred_mean))
        ax.plot(x, target_mean, color="black", linewidth=1.4, linestyle=":", label="Target mean")
        ax.plot(x, pred_mean, color=colour, linewidth=2.0, label="Total prediction mean")
        ax.plot(x, best_mean, color="#3f9e6e", linewidth=1.8, label=f"Best {self.percent:g}% mean")
        ax.plot(x, worst_mean, color="#e85651", linewidth=1.8, label=f"Worst {self.percent:g}% mean")
        ax.set_xlabel("Energy")
        ax.set_ylabel("Intensity")
        ax.legend(fontsize=8, framealpha=0.9)
        style_axis(ax)
    ...
    def _spread_figure(
        ...
        ax.plot(x, target_std, color="gray", linewidth=1.8, label="Target std")
    ...
    def _combined_mean_overlay(methods: list[_MethodMeans], out: Path) -> None:
        ...
        black dotted line; otherwise one dotted target curve per method is
    ...
    def _combined_tail_overlay(self, methods: list[_MethodMeans], out: Path, direction: str) -> None:
        ...
        for label_lines, colour, target_mean, pred_mean, _, _, best_mean, worst_mean in methods:
        ...
        MeanSpectrumPlotter._legend_with_styles(
            ax,
            [
                (f"{direction.capitalize()} {self.percent:g}% mean", "-"),
