...
"""Primitive building blocks (RBF, cutoffs, MLP, IrrepNorm) shared across E3EEFull layers."""
...
class CosineCutoff(nn.Module):
    ...
    Args:
        cutoff: Cutoff radius in **A** beyond which the envelope is zero.
    """
    ...
    def forward(self, r: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            r: Interatomic distances of arbitrary shape ``(...)`` in **A**.
...
class IrrepNorm(nn.Module):
    ...
    For ``l = 0`` blocks the multiplicity channels are mean/variance normalized.
    For ``l > 0`` blocks each irrep copy is RMS-normalized across its irrep dimension.
