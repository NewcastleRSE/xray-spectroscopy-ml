...
class ModelEvalTestSuite:
    def __init__(
        self, model, model_mode, trainloader, validloader, evalloader, n_in, out_dim
    ):
        self.model = model
        ...
        if self.model_mode == "mlp" or self.model_mode == "cnn":
            ...
            for inputs, labels in self.evalloader:
                ...
                (
                    ...
                    pred_x,
                    pred_y,
                ) = self.model.reconstruct_all_predict_all(
    ...
    def get_loss_input_mean_sd_train(self):
        if self.model_mode == "mlp" or self.model_mode == "cnn":
            ...
            for _, labels in self.evalloader:
                ...
                mean_sd_input = self.mean_input.repeat(
                    labels.shape[0], 1
                ) + torch.normal(
            ...
            for inputs, labels in self.evalloader:
                ...
                mean_sd_output = self.mean_output.repeat(
                    labels.shape[0], 1
                ) + torch.normal(
    ...
    def get_loss_output_mean_sd_train(self):
        if self.model_mode == "mlp" or self.model_mode == "cnn":
            ...
            for inputs, labels in self.evalloader:
                ...
                recon_target = self.mean_input.repeat(
                    labels.shape[0], 1
                ) + torch.normal(
                    torch.zeros([labels.shape[0], self.n_in]), self.std_input
                )
                pred_target = self.mean_output.repeat(
                    labels.shape[0], 1
                ) + torch.normal(
                    torch.zeros([labels.shape[0], self.out_dim]), self.std_output
                )
