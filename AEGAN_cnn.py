...
class AEGAN_CNN(nn.Module):
	def __init__(self, **params):
		...
		self.gen_a_max_conv_blocks = self.get_max_conv_blocks(in_length = self.input_size_a)
		self.gen_b_max_conv_blocks = self.get_max_conv_blocks(in_length = self.input_size_b)

		self.gen_a_num_conv_blocks = self.num_conv_blocks if self.num_conv_blocks <= self.gen_a_max_conv_blocks else self.gen_a_max_conv_blocks
		self.gen_b_num_conv_blocks = self.num_conv_blocks if self.num_conv_blocks <= self.gen_b_max_conv_blocks else self.gen_b_max_conv_blocks

		self.gen_a_encoder_conv_output_size = self.get_conv_blocks_output_size(in_length = self.input_size_a, num_conv_blocks = self.gen_a_num_conv_blocks)
		self.gen_b_encoder_conv_output_size = self.get_conv_blocks_output_size(in_length = self.input_size_b, num_conv_blocks = self.gen_b_num_conv_blocks)

		self.gen_a_decoder_conv_output_size = self.get_dec_blocks_output_size(num_conv_blocks = self.gen_a_num_conv_blocks)
		self.gen_b_decoder_conv_output_size = self.get_dec_blocks_output_size(num_conv_blocks = self.gen_b_num_conv_blocks)


		# ---------------------------------------- Initialise generative networks ---------------------------------------- #
		# Autoencoder for domain A
		self.gen_a = AEGenCNN(input_size = self.input_size_a, num_hidden_layer = self.n_hl_gen, hidden_size = self.hidden_size, dropout = self.dropout, activation = self.activation, 
							out_channel = self.out_channel, channel_mul = self.channel_mul, kernel_size = self.kernel_size, stride = self.stride, pooling_layer = self.pooling_layer,
							num_conv_blocks = self.gen_a_num_conv_blocks, enc_conv_block_output_size = self.gen_a_encoder_conv_output_size,enc_shared_hidden_size = self.hidden_size,
							dec_conv_block_output_size = self.gen_a_decoder_conv_output_size)
		# Autoencoder for domain B
		self.gen_b = AEGenCNN(input_size = self.input_size_b, num_hidden_layer = self.n_hl_gen, hidden_size = self.hidden_size, dropout = self.dropout, activation = self.activation, 
							out_channel = self.out_channel, channel_mul = self.channel_mul, kernel_size = self.kernel_size, stride = self.stride, pooling_layer = self.pooling_layer,
							num_conv_blocks = self.gen_b_num_conv_blocks, enc_conv_block_output_size = self.gen_b_encoder_conv_output_size, enc_shared_hidden_size = self.hidden_size,
							dec_conv_block_output_size = self.gen_b_decoder_conv_output_size)

		# Shared autoencoder layers
		self.enc_shared = SharedLayer(num_hidden_layer = self.n_hl_shared, hidden_size = self.hidden_size, dropout = self.dropout, activation = self.activation)
		self.dec_shared = SharedLayer(num_hidden_layer = self.n_hl_shared, hidden_size = self.hidden_size, dropout = self.dropout, activation = self.activation)

		# ---------------------------------------- Initialise discriminator networks ---------------------------------------- #
		self.dis_a = Dis(input_size = self.input_size_a, num_hidden_layer = self.n_hl_dis, hidden_size = self.hidden_size,dropout = self.dropout, activation = self.activation, loss_fn = self.loss_fn_dis)  # discriminator for domain a
		self.dis_b = Dis(input_size = self.input_size_b, num_hidden_layer = self.n_hl_dis, hidden_size = self.hidden_size,dropout = self.dropout, activation = self.activation, loss_fn = self.loss_fn_dis)  # discriminator for domain b
		...
		params_gen = [param for name, param in self.named_parameters() if 'dis' not in name]
	...
	def dis_update(self, x_a, x_b):
		...
		loss_fake = 0.25*(loss_gen_adv_a  + loss_gen_recon_a + loss_gen_adv_b + loss_gen_recon_b)
...
# class SharedLayerCNN(nn.Module):
# 	# Autoencoder architecture
# 	def __init__(self, num_hidden_layer, hidden_size, dropout, activation):
# 		super().__init__()
# 		self.num_hidden_layer = num_hidden_layer
...
class AEGenMLP(nn.Module):
	# Autoencoder architecture
	def __init__(self, input_size, num_hidden_layer, hidden_size, dropout, activation, pool):
		super().__init__()
...
class AEGenCNN(nn.Module):
	# Autoencoder architecture
	def __init__(self, input_size, num_hidden_layer, hidden_size, dropout, activation, out_channel, channel_mul, kernel_size, stride, pooling_layer,num_conv_blocks, enc_conv_block_output_size,enc_shared_hidden_size,dec_conv_block_output_size):
		super().__init__()
		self.input_size = input_size
		self.output_size = input_size
		self.num_hidden_layer = num_hidden_layer
		...
		self.enc_conv_block = []
		# input layer
		self.enc_conv_block.append(nn.Conv1d(in_channels = 1, out_channels = self.out_channel, kernel_size = self.kernel_size, stride = self.stride))
		self.enc_conv_block.append(self.activation)
		# mid layers
		for block in range(self.num_conv_blocks - 1):
			in_channel=int(out_channel)
			out_channel=int(out_channel * channel_mul)
			conv_layer = nn.Conv1d(in_channel,out_channel,kernel_size=self.kernel_size,stride=self.stride)
			if (block + 1) != int(self.num_conv_blocks-1):
				self.enc_conv_block.append(conv_layer)
				self.enc_conv_block.append(self.activation)
			else:
				self.enc_conv_block.append(conv_layer)
				self.enc_conv_block.append(self.pooling_layer)
				self.enc_conv_block.append(self.activation)
		...
		self.max_channel = (self.num_conv_blocks-1) * self.out_channel * self.channel_mul
		self.dec_conv_block = []
		# input layer
		out_channel = self.max_channel
		self.dec_conv_block.append(nn.ConvTranspose1d(in_channels = 1, out_channels = out_channel, kernel_size = self.kernel_size, stride = self.stride))
		self.dec_conv_block.append(activation)
		# mid layers
		for block in range(self.num_conv_blocks - 1):
			in_channel = int(out_channel)
			out_channel = int(out_channel / channel_mul)
			convtranspose_layer = nn.ConvTranspose1d(in_channel, out_channel, kernel_size = self.kernel_size, stride = self.stride)
			if (block + 1) != int(self.num_conv_blocks):
				self.dec_conv_block.append(convtranspose_layer)
				self.dec_conv_block.append(self.activation)

		self.dec_conv_block.append(nn.ConvTranspose1d(in_channels = out_channel, out_channels = 1, kernel_size = self.kernel_size, stride = self.stride))
		self.dec_conv_block = nn.Sequential(*self.dec_conv_block)
		# # ---------------------------

		# ---------------------------
		# DECODER ORIGINAL MLP LAYER
		dec_layer = []
		for layer in range(num_hidden_layer):
			dec_layer.append(nn.Linear(self.hidden_size,self.hidden_size))
			dec_layer.append(nn.BatchNorm1d(self.hidden_size))
...
class Dis(nn.Module):
	...
	def __init__(self, input_size, num_hidden_layer, hidden_size, dropout, activation, loss_fn):
		super().__init__()
		self.input_size = input_size
		self.num_hidden_layer = num_hidden_layer
		self.hidden_size = hidden_size
...
class ActivationSwitch:
	def fn(self, activation):
		default = nn.PReLU()
		return getattr(self, f"activation_function_{activation.lower()}", lambda: default)()
...
class EMDLoss(nn.Module):
	...
	def forward( self, y_true, y_pred):
		loss = torch.mean(torch.square(torch.cumsum(y_true, dim=-1) - torch.cumsum(y_pred, dim=-1)), dim=-1).sum()
		return loss
...
def train_aegan_cnn(x, y, hyperparams, n_epoch):
	...
	model = AEGAN_CNN(dim_a = hyperparams['input_size_a'],
	...
	for epoch in range(n_epoch):
		...
		for inputs_x, inputs_y in trainloader:
			...
			recon_x, recon_y, pred_x, pred_y  = model.reconstruct_all_predict_all(inputs_x,inputs_y)
		...
		print(f">>> Epoch {epoch}...")
		print(f'>>> Running reconstruction loss (structure) = {running_loss_recon_x:.4f}')
		print(f'>>> Running reconstruction loss (spectrum) =  {running_loss_recon_y:.4f}')
		print(f'>>> Running prediction loss (structure) =     {running_loss_pred_x:.4f}')
		print(f'>>> Running prediction loss (spectrum) =      {running_loss_pred_y:.4f}')

	losses = {'train_loss': train_total_loss, \
